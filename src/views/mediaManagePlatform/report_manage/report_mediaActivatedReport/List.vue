  ...
  data(){
    return {
      ...
      datagrid: {
        List: [],
        FieldList: [
          { key: 'mediaName', label: '媒体名称',width:360,tooltip:true,formatter:function(row,column,cellValue){
             return tool.str.textLength(row.mediaName,20)
          }},
          { key:'mediaIsActivated', label:'移动端是否激活',width:100,formatter:function(y,column,cellValue){
            return y.mediaIsActivated==1 ? '是': '否'
          }},
          { key:'salesmanNum', label:'促销员数量',width:100},
          { key:'salesmanActivatedNum', label:'促销员激活数量',width:100},
  ...
  methods: {
    onClickBtnDownload() {
      ...
      this.api_mediaManagePlatform_activatedDownLoad({
          ...
          a.download = '媒体激活数据'+dateTime + '.xls' ;
          a.href = down_url;
          $("body").append(a);    
          a.click();
          $(a).remove();
