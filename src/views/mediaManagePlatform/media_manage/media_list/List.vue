    ...
    <!--工具条-->
        <el-col :span="24" class="toolbar">
            <el-form :inline="true" :model="form">
                ...
                <el-form-item>
                    <el-select v-model="form.contractStatus"  placeholder="选择合约状态"  clearable >
                ...
                <el-form-item>
                    <el-select v-model="form.cooperWay"  placeholder="选择合作方式"  clearable >
        ...
        <list-data
          ...
          @onClickBtnMedia="onClickBtnMedia"
          @onChangeCurrentPage="onChangeCurPage"
          @onChangePageSize="onChangeCurPageSize"
          :DataGrid = 'datagrid'></list-data>
        <Contract :isShow="show_contract_dialog" :data="data" ></Contract>
        <Media :isShow="show_media_dialog" :data="data" ></Media>
...
export default {
  components:{EleMediaCategory,Contract,Media,Edit},
  data(){
    let tool = this.tool
    return{
      datagrid:{
        ...
        FieldList: [
          ...
          { type: 'operate', label: '操作',width:150,
            btns:[
              ...
              {text:'合同信息',type:'edit',eventName:'contract'}
            ]
          },
        ],
        ToolBar:[
          ...
          {text:'媒体信息',eventName:'onClickBtnMedia'},
        ],
        Pagination:{
          total:0,
          page_size:10,
  ...
  methods:{
    ...
    onClickBtnMedia(data){
      if( data.batch.datas.length !== 1){
          this.$message({type:'error',message:'请选择一项'})
          return
        }else{
          this.media( {data:data.batch.datas[0]} )
        }
    },
    edit(row){
      this.data = row.data
    ...
    init(){
      if( this.time && this.time.length > 0){
        ...
        if(this.time[1].indexOf(':') ==-1){
          this.form.signDateEnd = new Date(this.time[1]+' 23:59:59').getTime();
        }else {
