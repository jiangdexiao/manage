        ...
        <div class="project-list pad0">
            <div class="p_item">
                ...
                <div class="p_con_box">
                    ...
                    <div class="w50 fr">
                        <p class="p_text bg_c10">我的排名</p>
                        <p class="text_num text_c"><span>NO.</span>&nbsp;{{rankingNo}}</p>
                    </div>
                </div>
            </div>
        </div>
   ...
   data() {
    ...
    methods: {
        getDataInfo () {
            let that = this ;
            //获取媒体信息
            this.api_mediaManagePlatform_staff({data:{token: that.token,
                    bdStaffId: that.getUser.bd.bdId,}}).then(result=>{
            ...
            //我的排名
            this.api_mediaSaleManagePlatform_bdInfo({data:{token: that.token, bdId: that.getUser.bd.bdId}}).then(result=>{
              that.rankingNo = result.content && result.content.rankingNo ? result.content.rankingNo :'0';
            })
            //本月提成   
            this.api_mediaSaleManagePlatform_bdAccounting({data:{systemAccessToken: that.token, userId: that.getUser.bd.bdId, userType: 1}}).then(result=>{
              that.curMonthAmount = this.tool.str.twoDecimal(result.content.curMonthAmount,4); 
            })
