        ...
        <el-dialog custom-class="dialog" :title="form.mresId?'编辑广告位':'新增广告位'" @close="handleClose" :visible.sync="show_edit_dialog" :close-on-click-modal= "false" >
            <el-form ref="form" :model="form"  :rules="formRules" label-width="150px" >
                <el-form-item label="所属媒体" prop="mediaId" >
                     <el-select v-model="form.mediaId" placeholder="请选择媒体" @change="getAaddress" clearable >
                      <el-option v-for="(item,i) in mediaList" :key="i" :label="item.mediaName" :value="item.mediaId">
                      </el-option>
                    </el-select>
                </el-form-item>
...
export default {
  ...
  data(){
      const rule_sum = (rule,value,callback)=>{
          if( !this.tool.validate.numRange(value,1,200) ){
