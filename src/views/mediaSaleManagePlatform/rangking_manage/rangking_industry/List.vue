<template>
    <div class="list">
        ...
        <el-col :span="24" class="toolbar" style="height:50px;line-height:50px;">
            ...
            <el-col :span="3">行业排名  第{{ bdInfo.rankingNo }}名  </el-col>
...
export default {
    ...
    methods: {
        // 获取业务进度列表
        init() {
          this.api_mediaSaleManagePlatform_bdRanking({data: {cate:this.getUser.bd.cateCode!=null?this.getUser.bd.cateCode:''} }).then(res=>{
            this.datagrid.List = res.content?res.content:[]
            // this.datagrid.Pagination.total = res.totalCount
          })
        },
        getBdInfo(){
          this.api_mediaSaleManagePlatform_bdInfo({ data:{bdId:this.getUser.bd.bdId,cate:this.getUser.bd.cateCode!=null?this.getUser.bd.cateCode:''} }).then(res=>{
            if( res.content ){
