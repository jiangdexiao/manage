<template>
    <div class="list">
        ...
        <el-col :span="24" class="toolbar" style="height:50px;line-height:50px;">
            ...
            <el-col :span="3">区域排名  第{{ bdInfo.rankingNo }}名  </el-col>
...
export default {
        ...
        return {
          datagrid:{
            ...
            FieldList: [
              ...
              { key: 'monthCompare', label: '所属组织',formatter:function(row,column,cellValue){
                let addr = ''
    ...
    methods: {
        // 获取业务进度列表
        init() {
          this.api_mediaSaleManagePlatform_bdRanking({data: {regionCode:this.getUser.bd.areaCode!=null?this.getUser.bd.areaCode:''} }).then(res=>{
            this.datagrid.List = res.content?res.content:[]
            // this.datagrid.Pagination.total = res.totalCount
          })
        },
        getBdInfo(){
          this.api_mediaSaleManagePlatform_bdInfo({ data:{bdId:this.getUser.bd.bdId,regionCode:this.getUser.bd.areaCode!=null?this.getUser.bd.areaCode:''} }).then(res=>{
            if( res.content ){
