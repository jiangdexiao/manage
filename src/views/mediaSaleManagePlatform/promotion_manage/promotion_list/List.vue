...
<script>
  ...
  export default {
      ...
      return {
        datagrid:{
          ...
          FieldList: [
            ...
            { key: 'roleType', label: '角色',width:100,formatter:function(row,column,cellValue){ return row.roleType === 1?'BD负责人':'普通专员' }},
            ...
            { key: 'lastLoginDate', label: '最后登录',width:150,formatter:function(row,column,cellValue){
              return tool.date.format(row.lastLoginDate,'yyyy-MM-dd hh:mm')
            }},
            { key: 'createDate', label: '新增时间',width:150,formatter:function(row,column,cellValue){
              return tool.date.format(row.createDate,'yyyy-MM-dd hh:mm')
            }},
            { type:'operate',label: '操作',width:100,
              btns:[
                {text:'修改',eventName:'edit'},
                {text:'删除',type:'danger',eventName:'del',condition:( row )=>{
                  if(row.data.bdId == this.getUser.bd.bdId){
                    return false
                  }
                  return true
                }}
    ...
    methods: {
      ...
      //删除
      del(opts) {
        if(opts.data.bdId == this.getUser.bd.bdId){
          this.$message({type:'error',message:'当前用户不能删除'})
          return  
        }
