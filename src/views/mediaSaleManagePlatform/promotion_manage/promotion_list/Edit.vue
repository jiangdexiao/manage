            ...
            <el-col :span="24" >
                <el-form ref="form" :model="form" :rules="formRules"  label-width="100px">
                    ...
                    <el-form-item label="所属组织" prop="orgId">  
                        <EleOrgProvinceCity :disabled="true" :selectVal="orgs" v-on:selected="EleOrgProvinceCity_CallBack"></EleOrgProvinceCity>
                    </el-form-item>
...
export default {
  ...
  data(){
      ...
      return{
          ...
          formRules:{
              ...
              regionCode:[
                  {required:true,message:'请选择区域信息',trigger:'change'}
              ],
              mediaCate:[
  ...
  methods:{
    get_bdList(query){
      ...
      this.loading = true;
      this.api_mediaManagePlatform_bd({data:{orgId:this.queryBdId,orgType:1,keyWords:query,operateBdId:this.getUser.bd.bdId}}).then(res=>{
        this.loading = false;
        this.bdList = res.content
      })
    },
    EleOrgProvinceCity_CallBack(val,parentId,orgLevel,array){
        this.form.orgId = val;
        this.orgsTemp = array
        ...
        if( parentId &&  parentId != this.queryBdId && orgLevel!=0){
            ...
            this.api_mediaManagePlatform_bd({data:{orgId:parentId,orgType:1,operateBdId:this.getUser.bd.bdId}}).then(res=>{
                this.bdList = res.content
            })
        }else if(orgLevel==0){
            this.bdList = [];
    ...
    save(){
        let len = this.orgsTemp.length
        if( this.pageOption==='edit' ){
            //编辑时候
            if( len === 0 || len <= this.getUser.bd.orgInfoList.length){
                if( this.form.bdId !== this.getUser.bd.bdId){
                    this.$message({type:'error',message:'请选择一个当前所属级别的下级'})
                    return
                }
            }
        }else{
    ...
    clear_param() {
        ...
        this.bdList=[]; this.orgs = [];this.orgsTemp=[]; this.areas=[];  this.medias=[];
    },
    getBdId(orgLevel,ListData) {
        let bdId='',operateBdId='';
        if(orgLevel>0){
        ...
        this.queryBdId = bdId;
        if( this.getUser.bd.bdId == this.form.bdId ){
            operateBdId = ''
        }else{
            operateBdId = this.getUser.bd.bdId
        }
        this.api_mediaManagePlatform_bd({data:{orgId:bdId,orgType:1,operateBdId:operateBdId}}).then(res=>{
            this.bdList = res.content
        })
    }
  },
          ...
          if( v === true ){
            ...
            if( this.data && this.data.bdId ){
                ...
                this.api_mediaManagePlatform_bd({pathParams:`/${this.data.bdId}`}).then(res=>{
                    ...
                    this.form.sex = c.sex
                    this.form.age = c.age
                    this.form.orgId = c.orgId
                    this.orgs = c.orgInfoList.map(item=>{
                        return item.orgId
                    })
                    this.orgsTemp = this.orgs
                ...
                if( this.getUser && this.getUser.bd.orgInfoList && this.getUser.bd.orgInfoList.length>0){
                    this.bdOrgLevel = this.getUser.bd.orgInfoList.length-1;
                    this.getBdId(this.getUser.bd.orgInfoList[this.getUser.bd.orgInfoList.length-1].orgLevel, this.getUser.bd.orgInfoList);
                    setTimeout(() => {
                        this.form.orgId = this.getUser.bd.orgInfoList[this.bdOrgLevel]
