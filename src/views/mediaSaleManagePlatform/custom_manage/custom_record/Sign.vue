<template>
  <div class="edit-form">
                    ...
                    <!-- <el-form-item label-width="0"> -->
                        <el-col :span="12" >
                            ...
                            <el-form-item label="详细地址" prop="detailAddr">
                                <el-input type="textarea" v-model="form.detailAddr" :maxlength="100" ></el-input>
                            </el-form-item>
    ...
    <SignList :isShow="show_signuserlist_dialog" :form="signForm"></SignList>
  </div>
</template>
<script>
...
export default {
  components:{EleMediaCategory,EleMap,UserList,EditUser,SignList},
  data(){
      let _this = this
      let tool = this.tool
  ...
  methods:{
    ...
    //媒体分类
    EleMediaCategory_CallBack(val,varArr){
        this.form.mediaCate = varArr.length==2? val :''
    },
    handleClose(){
        this.show_edit_dialog = false
        this.$parent.show_sign_dialog = false;
              ...
              //签约
                this.api_common_mediaId({data:{prefix:this.prefix_18MD}}).then(res=>{
                    ...
                    type:'post',
                        ...
                        this.handleClose()
                        //显示签约客户列表
                        this.show_signuserlist_dialog = true
      ...
      isShow(v){
          ...
          if( v === true){
              if( this.data && this.data.recordId ){
                  this.api_mediaManagePlatform_record({pathParams:`/${ this.data.recordId }`}).then(res=>{
                      console.log(res.content)
                      let date = new Date()
                      let beginTime = this.tool.date.format(date, "yyyy-MM-dd")
                      let time = (date.getFullYear()+3)+"-"+(date.getMonth()+1)+"-"+date.getDate()
                      let endTime = this.tool.date.format(new Date(time), "yyyy-MM-dd")
                      let media = res.content
                      this.tool.dp.assignData(this.getLeveldata, res.content)
                      this.form.mediaName = media.mediaName?media.mediaName:''
                      this.form.tagAddr = media.mediaTagAddr?media.mediaTagAddr:''
                      this.form.latitude = media.mediaLatitude?media.mediaLatitude:''
                      this.form.longitude = media.mediaLongitude?media.mediaLongitude:''
                      this.form.detailAddr = media.address
                      this.form.contactMan = media.contactName
                      this.form.contactTel = media.contactMobile
                      this.form.mediaParCate = media.mediaParentCateCode
                      this.form.mediaCate = media.mediaCateCode
                      this.form.mresDisc = media.describe
                      this.form.dateStart = beginTime
                      this.form.dateEnd = endTime
                      this.form.effectDate = beginTime
                      this.form.bdStaffId = media.bdStaffId
                      this.medias.push(media.mediaParentCateCode)
                      this.medias.push(media.mediaCateCode)

                      this.form.contractMresNum = media.contractMresNum
                    //   this.form.mresCate = media.mresCate?JSON.parse(media.mresCate):[]
                      
                      this.form.bdStaffNo = media.bdStaffNo
                      this.form.bdStaffName = media.bdStaffName
                    ...
                    //   this.form.level = media.level
                    //   this.form.busiArea = media.busiArea,//营业面积
                    //   this.form.hourStart = media.hourStart,//营业时间
                    //   this.form.hourEnd = media.hourEnd,
                    //   this.form.busyInfo = media.busyInfo,
                    //   this.form.workDayCusts = media.workDayCusts,
                    //   this.form.weekDayCusts = media.weekDayCusts,
                    //   this.form.averageConsu = media.averageConsu,
                    //   this.form.employeeNum = media.employeeNum,

                      this.form.time.push(beginTime)
                      this.form.time.push(endTime)
                      this.signForm.keyword = media.keywords?media.keywords:''
                      this.signForm.recordName = media.recordName?media.recordName:''
