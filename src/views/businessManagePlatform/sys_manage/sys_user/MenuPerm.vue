...
export default {
  ...
  props: ["menuShow", "userId", "menuPerList" , "menuPerListChecked"],
  methods: {
    handleClose () {
    ...
    save () {
      ...
      selnodes.map((item, i) => {
        for (let k = 0; k < menus.length; k++) {
          if (item.moduleId === menus[k].moduleId && JSON.stringify(menus).indexOf(item.parentId) === -1 && item.parentId !== '0') {
      ...
      $.ajax ({
        type: 'post',
        url: this.BASE_URL + 'm2c.operate/user/auth/' + this.userId + '/menus',
