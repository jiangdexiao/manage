        ...
        <el-form :inline="true" class="demo-form-inline">
        	<div class="addnewcon">
	         	<el-form-item class="clear02">
		         	<span class="mr20 fl con01 mt5">
		         		<div class="tit">新人礼包</div>
		         		<div class="tit">名称</div>
		         	</span>
		         	<el-input class="fl input360 mr20 mt5" v-model="giftPacketParams.name" placeholder="不超过11个字" :maxlength="22" @blur="formValidator(1)"></el-input>
		         	<el-button class="mr20" type="primary" size="small" @click="addZone()" v-if="btnShow === true">+ 添加专区</el-button>
		         	<span class="mr20 yellowcolor mt5">最多添加3个专区</span>
		  			</el-form-item>
              ...
              <div class="bordert" v-for="(zone, index) in giftPacketParams.zoneList">
                <div class="add_a_top borderl">
                  ...
                  <div>
                    <!--未选择优惠券的时候-->
                    <div class="icon_quan" v-if="zone.coupon_id == ''" @click="openCouponDialog(index, false)">
                      <div class="jiahao">+</div>
                      <div>选择优惠券</div>
                    </div>
                    <!--选择优惠券的时候-->
                    <div class="clear02 mr10 coupon_box f-active" v-if="zone.coupon_id != ''" @click="openCouponDialog(index, zone.isOld)">
                  ...
                  <div>
                    ...
                    <div class="goods_box clear02" v-if="zone.coupon_id != ''">
                      <div v-if="(index == 0 && all_show_0 == true) || (index == 1 && all_show_1 == true) || (index == 2 && all_show_2 == true)">
                        <div v-for="(range, index1) in zone.range_list" class="wose fl">
                      ...
                      <div v-if="(index == 0 && all_show_0 == false) || (index == 1 && all_show_1 == false) || (index == 2 && all_show_2 == false)">
                        <div v-for="(range, index1) in zone.range_list" v-if="index1 < 10" class="wose fl">
                          <el-tooltip v-if="zone.range_type != 1" :content="range.goodsName" placement="top-start" effect="light">
                            <div class="img fl mr10 mb10">
                              <img :src="range.goodsImageUrl"/>
                            </div>
	        ...
	        <div class="addnewcon mt10">
	         	<div>
                  ...
                  <el-option value="1" label="不限制"></el-option>
                  <el-option value="2" label="一周内"></el-option>
                  <el-option value="3" label="一个月内"></el-option>
                  <el-option value="4" label="三个月内"></el-option>
                  <el-option value="5" label="半年内"></el-option>
                  <el-option value="6" label="该礼包生效后注册"></el-option>
						    </el-select>
		         	</el-form-item>
	         	</div>
	         	<el-form-item>
		         	<span class="mr30 fl con01">
		         		<div class="tit">是否需要<br/>领取</div>
		         	</span>
              <span v-if="giftPacketParams.receiveType == '1'">需要领取</span>
		         	<span v-if="giftPacketParams.receiveType == '0'">不需要领取</span>
		  			</el-form-item>
	        </div>
	        <div class="addnewcon mt10">
	         	<el-form-item class="clear02">
      ...
      <el-dialog title="选择优惠券" :visible.sync="couponDialogVisible" width="930px" center>
          ...
          <div class="Choice_coupon_b clear02" v-if="couponList.length === 0">
            ...
            <p style="margin-left:330px;margin-top:20px;color:#CCCCCC;">暂时还没有商品可供选择哦</p>
			  	...
			  	<div class="Choice_coupon_b clear02" v-if="couponList.length !== 0">
			  		<div class="fl mr10 coupon_box f-active" :class="coupon.isChoosed == 1 ? 'f-active' : ''" v-for="(coupon,index) in couponList" @click="chooseCoupon(coupon)">
			  			<i class="icon_selected" v-if="coupon.isChoosed == 1"></i>
			  			<div class="coupon_box_left fl">
      ...
      <!--预览弹框-->
          ...
          <div class="content01">
            ...
            <div class="content_b02 clear02">
              <div class="mb10 word_break" v-for="(rule, index) in ruleList">
  ...
  export default {
    data () {
      return {
        ...
        currentZoneIndex: '',
        ruleList: [],
        oldDialogImg: {url: '', plus: ''},
        oldBannerImg: {url: '', plus: ''},
        oldBackgroundImg: {url: '', plus: ''},
        giftPacketParams: {
          name: '',
          effective: '',
          receiveType: '',
    ...
    mounted () {
      ...
      _this.api_businessManagePlatform_packetDetail({
        ...
        let packetInfo = result.content
        _this.giftPacketParams.name = packetInfo.packetName
        _this.giftPacketParams.effective = packetInfo.effectiveUser.toString()
        _this.giftPacketParams.receiveType = packetInfo.receiveType.toString()
        _this.giftPacketParams.withOtherCut = packetInfo.withOtherCut.toString()
        _this.giftPacketParams.tip = packetInfo.tip
        _this.giftPacketParams.personalEdit = packetInfo.personalEdit.toString()
        _this.giftPacketParams.dialogImg = packetInfo.pageUrlPlus
        _this.giftPacketParams.bannerImg = packetInfo.bannerUrlPlus
        _this.giftPacketParams.backgroundImg = packetInfo.bgPicUrlPlus
        _this.giftPacketParams.dialogPreImg = packetInfo.pageUrl
        _this.giftPacketParams.bannerPreImg = packetInfo.bannerUrl
        _this.giftPacketParams.backgroundPreImg = packetInfo.bgPicUrl
        _this.oldDialogImg.url = packetInfo.pageUrl
        _this.oldBannerImg.url = packetInfo.bannerUrl
        _this.oldBackgroundImg.url = packetInfo.bgPicUrl
        _this.oldDialogImg.plus = packetInfo.pageUrlPlus
        _this.oldBannerImg.plus = packetInfo.bannerUrlPlus
        _this.oldBackgroundImg.plus = packetInfo.bgPicUrlPlus
        if (packetInfo.zonelist != null && packetInfo.zonelist.length > 0) {
          for (let i = 0, size = packetInfo.zonelist.length; i < size; i++) {
            let zoneJosn = {}
            zoneJosn.zone_id = packetInfo.zonelist[i].zoneId
            zoneJosn.zone_name = packetInfo.zonelist[i].zoneName
            zoneJosn.coupon_id = packetInfo.zonelist[i].couponDetailRepresentation.couponId
            zoneJosn.coupon_name = packetInfo.zonelist[i].couponDetailRepresentation.couponName
            zoneJosn.range_type = packetInfo.zonelist[i].couponDetailRepresentation.rangeType
            zoneJosn.range_content = packetInfo.zonelist[i].couponDetailRepresentation.rangeContent
            zoneJosn.coupon_form = packetInfo.zonelist[i].couponDetailRepresentation.couponForm
            zoneJosn.coupon_value = packetInfo.zonelist[i].couponDetailRepresentation.value
            zoneJosn.threshold_content = packetInfo.zonelist[i].couponDetailRepresentation.thresholdContent
            zoneJosn.coupon_timeStart = packetInfo.zonelist[i].couponDetailRepresentation.expirationTimeStart
            zoneJosn.coupon_timeEnd = packetInfo.zonelist[i].couponDetailRepresentation.expirationTimeEnd
            zoneJosn.range_count = packetInfo.zonelist[i].entityNum
            zoneJosn.range_list = packetInfo.zonelist[i].rangeList
            zoneJosn.isOld = true
    ...
    methods: {
      ...
      addZone () {
        ...
        if (_this.giftPacketParams.zoneList.length < 3) {
          ...
          if (flag === false) {
            ...
            zoneJosn.coupon_id = ''
            zoneJosn.zone_name = ''
            zoneJosn.isOld = false
            _this.giftPacketParams.zoneList.push(zoneJosn)
          }
        } else {
          _this.warning('最多新增3个专区，请删除后再添加')
        }
      },
      // 删除一个专区
      deleteZone (index) {
      ...
      selectCoupon () {
        ...
        _this.api_businessManagePlatform_activityCouponPage({
          ...
          for (let i = 0, size = _this.couponList.length; i < size; i++) {
            _this.couponList[i].isChoosed = 0
            for (let j = 0; j < _this.giftPacketParams.zoneList.length; j++) {
              if (_this.couponList[i].couponId === _this.giftPacketParams.zoneList[j].coupon_id) {
                _this.couponList[i].isChoosed = 1
              }
            }
          }
        })
...
//        })
      ...
      chooseCoupon (coupon) {
        let _this = this
        console.log('111111111', _this.couponList.length)
        if (coupon.isChoosed == 0) {
          for (let i = 0, size = _this.couponList.length; i < size; i++) {
            console.log(222)
            if (_this.couponList[i].couponId === _this.giftPacketParams.zoneList[_this.currentZoneIndex].coupon_id) {
              _this.couponList[i].isChoosed = 0
            }
          }
          coupon.isChoosed = 1
          ...
          _this.couponRangList(_this.currentZoneIndex, coupon.couponId)
          console.log(333)
        }
      },
      changeAllShow (index) {
      ...
      handleDialogImgChange (e) {
        ...
        _this.beforeUpload(file, 'dialogImg')
      },
      // 选择banner图片
      selectBannerImg () {
        let _this = this
      ...
      handleBannerImgChange (e) {
        ...
        _this.beforeUpload(file, 'bannerImg')
      },
      // 选择背景图片
      selectBackgroundImg () {
        let _this = this
      ...
      handleBackgroundImgChange (e) {
        ...
        _this.beforeUpload(file, 'backgroundImg')
      },
      // 上传前验证
      beforeUpload (file, flag) {
        ...
        if (size > 600) {
          _this.$message.error('上传图片大小最大600K!')
          return false
        }
        let image = new Image()
        ...
        image.onload = function () {
          ...
          if (flag === 'dialogImg') {
            if (image.width !== 630 || image.height !== 750) {
              _this.$message.error('上传图片尺寸限制为630x750!')
            } else {
          ...
          if (flag === 'backgroundImg') {
            if (image.width !== 750 || image.height !== 1380) {
              _this.$message.error('上传图片尺寸限制为750x1380!')
            } else {
      ...
      postfrom () {
        ...
        if (flag) {
          ...
          let rebody = {
            packet_id: sessionStorage.getItem('packet_id'),
          ...
          _this.api_businessManagePlatform_packetModify({
...
//          $.ajax({
//            url: _this.BASE_URL + 'm2c.market/admin/packet/modify',
...
//          })
      ...
      back () {
        let _this = this
        if (confirm('确定放弃新增新人礼包吗？该内容不会保存')) {
          _this.$router.push({path: '/m/marketActivity/giftPacketList'})
        }
      },
      // 确认个性设置
      ...
      cancelSetting () {
        ...
        if (confirm('确定放弃编辑吗?该内容不会保存')) {
          _this.giftPacketParams.dialogImg = _this.oldDialogImg.plus
          _this.giftPacketParams.bannerImg = _this.oldBannerImg.plus
          _this.giftPacketParams.backgroundImg = _this.oldBackgroundImg.plus
          _this.giftPacketParams.dialogPreImg = _this.oldDialogImg.url
          _this.giftPacketParams.bannerPreImg = _this.oldBannerImg.url
          _this.giftPacketParams.backgroundPreImg = _this.oldBackgroundImg.url
          _this.settingDialogVisible = false
        }
      },
      // 验证提示
...
//      },
      ...
      // 获取优惠券作用商品/店铺
      couponRangList (zoneIndex, couponId) {
        let _this = this
        let parmas = {couponId: couponId, pageNum: 1, rows: 30}
        _this.api_businessManagePlatform_couponApplyGoods({
          data: parmas
        }).then(result => {
          _this.giftPacketParams.zoneList[zoneIndex].range_count = result.totalCount
          _this.giftPacketParams.zoneList[zoneIndex].range_list = result.content
        })
//        $.ajax({
//          method: 'get',
//          url: _this.BASE_URL + 'm2c.scm/goods-out/coupon/apply',
//          data: parmas,
//          success: function (res) {
//            if (res.status == 200) {
//              _this.giftPacketParams.zoneList[zoneIndex].range_count = res.totalCount
//              _this.giftPacketParams.zoneList[zoneIndex].range_list = res.content
//            }
//          }
//        })
      }
