      ...
      <el-col :span="24" class="toolbar">
        <el-form :inline="true" class="demo-form-inline">
          ...
          <div class="BasicinFormation clear02">
            ...
            <div class="mt10 clear02">
              <el-form-item label="优惠券名称" class="ml20 fl">
                <el-tooltip placement="top-start" effect="light">
                  <div slot="content">
                    可用英文数字，汉字，特殊字符
                    <br/> 一个中文数字符号=1个字，一个英文=半个字
                    <br/>
                  </div>
                  <i class="icon-intro02 poi2" style="margin-top: initial;left: -104px;top: 18px;"></i>
                </el-tooltip>
                <el-input v-model="couponParams.coupon_name" placeholder="不超过11个字" :maxlength="22" @blur="formValidator('1')"></el-input>
              </el-form-item>
              <!--<span class="name_style fl"></span>-->
              <el-form-item label="面值" class="ml65 coupon_input">
                <el-tooltip placement="top-start" effect="light">
                  <div slot="content" v-if="couponParams.coupon_form == '1'">
                    必填项；可填写最高1000元
                    <br/> 1.输入相同金额则表示为固定金额
                    <br/> 2.输入不同区间值，用户将会领取区间中的随机金额
                    <br/>
                  </div>
                  <div slot="content" v-if="couponParams.coupon_form == '2'">
                    必填项；可填写1-9.9之间最多一位小数
                    <br/> 1.输入相同折扣则表示为固定折扣
                    <br/> 2.输入不同区间值，用户将会领取区间中的随机折扣
                    <br/>
                  </div>
                  <i class="icon-intro02 poi2" style="margin-top: initial;left: -61px;top: 18px;"></i>
                </el-tooltip>
                <!--当选择减钱的时候，出现的单位是元，当选择打折时候单位是折，请注意判断一下-->
                <el-select v-model="couponParams.coupon_form" placeholder="请选择">
                  <el-option label="减钱" value="1"></el-option>
                  <el-option label="打折" value="2"></el-option>
                </el-select>
              </el-form-item>
              <el-form-item v-if="couponParams.coupon_form == '1'" class="ml15 coupon_input">
                <el-input  v-model="couponParams.coupon_json.minMoney" :maxlength="4" @blur="formValidator('4')">
                  <i slot="suffix" class="el-input__icon fontstyle">元</i>
                </el-input>
              </el-form-item>
              <el-form-item v-if="couponParams.coupon_form == '2'" class="ml15 coupon_input">
                <el-input  v-model="couponParams.coupon_json.minDiscount" :maxlength="4" @blur="formValidator('4')">
              ...
              <el-form-item v-if="couponParams.coupon_form == '1'" class="coupon_input">
                <el-input  v-model="couponParams.coupon_json.maxMoney" :maxlength="4" @blur="formValidator('5')">
                  <i slot="suffix" class="el-input__icon fontstyle">元</i>
                </el-input>
              </el-form-item>
              <el-form-item v-if="couponParams.coupon_form == '2'" class="coupon_input">
                <el-input  v-model="couponParams.coupon_json.maxDiscount" :maxlength="4" @blur="formValidator('5')">
                  <i slot="suffix" class="el-input__icon fontstyle">折</i>
                </el-input>
              </el-form-item>
              <!--<div class="tit" v-if="couponParams.coupon_form == '1'">-->
                <!--必填项；可填写最高1000元；输入相同金额则表示为固定金额；输入不同区间值，用户将会领取区间中的随机金额。-->
              <!--</div>-->
              <!--<div class="tit" v-if="couponParams.coupon_form == '2'">-->
                <!--必填项；可填写1-9.9之间最多一位小数；输入相同折扣则表示为固定折扣；输入不同区间值，用户将会领取区间中的随机折扣。-->
              <!--</div>-->
            </div>
            <div class="pl28 ml20">
          ...
          <div class="BasicinFormation clear02">
            ...
            <div class="tit01 ml20" v-if="couponParams.threshold_type == 1 || couponParams.threshold_type == 2">
            ...
            <div v-if="couponParams.range_type == '1'">
              <el-col :span="20">
                <div class="shopbox">
                  <div class="shopbox_top clear02">
                    <div class="mt7 fl wid290">
                      <el-input placeholder="请输入店铺名称" v-model="shop_query_item.dealerName">
          ...
          <div class="BasicinFormation clear02">
            ...
            <div class="mt10">
              ...
              <el-button type="primary" size="medium" class="mt2" @click="openCostDailog()">单独成本设置</el-button>
          	  <el-col :span="20">
          ...
          <!--其他规则-->
          <div class="BasicinFormation clear02 mb100">
            <div class="Basicinbt">
        ...
        <el-dialog title="单独成本设置" :visible.sync="costDialogVisible" width="980px" center :close="scanShopCost">
          <div class="sigle_set clear02">
            <div class="sigle_set_left fl">
              ...
              <el-paginaton v-if="costShopResult.content != null && costShopResult.content.length > 0"
  ...
  data () {
    return {
      ...
      couponParams: {
        ...
        total_num: '', // 优惠券总数
        coupon_type: '3', // 优惠券类型，1：代金券，2：折扣券，3：分享券
        coupon_form: '1', // 优惠券形式，1：减钱，2：打折
        threshold_type: '1', // 门槛类型，1：金额，2：件数，3：无条件
        // threshold:门槛金额/件数，minMoney:优惠券面值最小值，maxMoney:优惠券面值最大值，minDiscount:优惠券折扣最小值，maxDiscount:优惠券折扣最大值
        coupon_json: {threshold: '', minMoney: '', maxMoney: '', minDiscount: '', maxDiscount: '', same: 0},
        ...
        remove_goods_ids: [],
        receive_type: '1',
        num_per_one: 0, // 每人限领总次数,0:不限制
        num_per_day: 0, // 每人每天限领次数,0:不限制
...
//        })
    ...
    formValidator (flag) {
      let _this = this
      // flag--1:名称，2：总数，3：时间,4:面值/折扣最小值，5：面值/折扣最大值,7:平台成本,8:商家成本，9：门槛
      if (flag === '0' || flag === '1') {
      ...
      if (flag === '0' || flag === '4') {
        if (_this.couponParams.coupon_form === '1') {
          if (!/^([1-9]\d{0,2}|1000)$/.test(_this.couponParams.coupon_json.minMoney)) {
            _this.couponParams.coupon_json.minMoney = ''
            _this.warning('优惠券面值为最多1000正整数')
            return false
          }
          if (_this.couponParams.coupon_json.minMoney !== '' && _this.couponParams.coupon_json.maxMoney !== '') {
            if (parseInt(_this.couponParams.coupon_json.minMoney) == parseInt(_this.couponParams.coupon_json.maxMoney)) {
              _this.couponParams.coupon_json.same = 1
            }
            if (parseInt(_this.couponParams.coupon_json.minMoney) > parseInt(_this.couponParams.coupon_json.maxMoney)) {
              _this.couponParams.coupon_json.minMoney = ''
              _this.warning('最小面值不能超过最大面值')
              return false
            }
          }
        }
        if (_this.couponParams.coupon_form === '2') {
          if (!/^[1-9]([.][0-9])?$/.test(_this.couponParams.coupon_json.minDiscount)) {
            _this.couponParams.coupon_json.minDiscount = ''
            _this.warning('优惠券折扣为1-9.9间一位小数')
            return false
          }
          if (_this.couponParams.coupon_json.minDiscount !== '' && _this.couponParams.coupon_json.maxDiscount !== '') {
            if (parseInt(_this.couponParams.coupon_json.minDiscount) == parseInt(_this.couponParams.coupon_json.maxDiscount)) {
              _this.couponParams.coupon_json.same = 1
            }
            if (parseFloat(_this.couponParams.coupon_json.minDiscount) > parseFloat(_this.couponParams.coupon_json.maxDiscount)) {
              _this.couponParams.coupon_json.minDiscount = ''
              _this.warning('优惠券最小折扣不能超过最大折扣')
              return false
            }
          }
        }
      ...
      if (flag === '0' || flag === '5') {
        if (_this.couponParams.coupon_form === '1') {
          if (!/^([1-9]\d{0,2}|1000)$/.test(_this.couponParams.coupon_json.maxMoney)) {
            _this.couponParams.coupon_json.maxMoney = ''
            _this.warning('优惠券面值为最多1000正整数')
            return false
          }
          if (_this.couponParams.coupon_json.minMoney !== '' && _this.couponParams.coupon_json.maxMoney !== '') {
          ...
          if (_this.couponParams.coupon_json.maxMoney !== '' && _this.couponParams.threshold_type === '1' && _this.couponParams.coupon_json.threshold !== '') {
            if (parseInt(_this.couponParams.coupon_json.maxMoney) >= parseInt(_this.couponParams.coupon_json.threshold)) {
              _this.couponParams.coupon_json.maxMoney = ''
              _this.warning('优惠券最大优惠金额不能大于优惠门槛')
              return false
            }
          }
        }
        if (_this.couponParams.coupon_form === '2') {
          if (!/^[1-9]([.][0-9])?$/.test(_this.couponParams.coupon_json.maxDiscount)) {
            _this.couponParams.coupon_json.maxDiscount = ''
            _this.warning('优惠券折扣为1-9.9间一位小数')
            return false
          }
          if (_this.couponParams.coupon_json.minDiscount !== '' && _this.couponParams.coupon_json.maxDiscount !== '') {
      ...
      if (flag === '0' || flag === '9') {
        if (_this.couponParams.threshold_type === '1' || _this.couponParams.threshold_type === '2') {
          ...
          if (_this.couponParams.threshold !== '' && _this.couponParams.threshold_type === '1' && _this.couponParams.coupon_form === '1' && _this.couponParams.coupon_json.maxMoney !== '') {
            if (parseInt(_this.couponParams.coupon_json.maxMoney) >= parseInt(_this.couponParams.coupon_json.threshold)) {
              _this.couponParams.coupon_json.threshold = ''
              _this.warning('优惠券最大优惠金额不能大于优惠门槛')
              return false
