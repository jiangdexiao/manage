...
export default {
  ...
  methods:{
    regionHandleItemChange(sel){
      ...
      let currentVal = sel[sel.length-1]
      this.$emit('selected',currentVal,sel);
      for(var i=0; i<sel.length; i++){
        if( i === 0){
            this.tempArea = this.list.find(p=> p.value == sel[i])
        }
        else{
            if(this.tempArea && this.tempArea.children){
                this.tempArea= this.tempArea.children.find(p=> p.value == sel[i])
            }else{
     ...
     if( sel && sel.length === 1){
         this.getCitys(currentVal,this.tempArea||this)
     }else if(sel && sel.length === 2){
         this.getAreas(currentVal,this.tempArea||this)
     }
    ...
    getAreas(val,obj){
        this.api_common_areas({ data:{city:val,token:this.getUser.token } }).then(res=>{
            if( res.content && res.content.length>0){
